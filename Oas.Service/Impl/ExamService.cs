...
namespace Oas.Service.Impl
{
    ...
    {
        private readonly OasContext _oasContext;
        ...
        public Paper GeneratePaper(string courseId, string style)
        {
            ...
            paper.MultipleQuestions =
                _oasContext.SelectableQuestions.Where(q => multiples.Contains(q.QuestionId)).ToList();
            paper.SubjectiveQuestions =
                _oasContext.SubjectiveQuestions.Where(q => subjectives.Contains(q.QuestionId)).ToList();
        ...
        public IEnumerable<AnswerSheet> GetStudentAnswerSheets(Guid studentId)
        {
            return _oasContext.AnswerSheets.Where(a => a.Student.MemberId == studentId);
        }

        public IEnumerable<AnswerSheet> GetUnevaluatedAnswerSheets(string courseId)
        {
            return _oasContext.AnswerSheets
                .Where(a => a.Course.CourseId == courseId)
                .Where(a => a.IsEnvaluated == false);
        }

        public void EvaluateAnswerSheet(Guid answerSheetId, AnswerSheetEnvaluation evaluation)
        {
            var answerSheet = _oasContext.AnswerSheets
                .Include("SelectableQuestionAnswers.Question")
                .Include("SubjectiveQuestionAnswers.Question")
            ...
            foreach (var subjectiveQuestionAnswer in answerSheet.SubjectiveQuestionAnswers)
            {
                var score = evaluation.GetScore(subjectiveQuestionAnswer.QuestionId);
                subjectiveQuestionAnswer.GotScore = score;
            }
